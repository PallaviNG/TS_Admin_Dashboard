...
import {
  saveAllBatchesOfInterviewerAction,
  saveAllInterviewersAction,
  saveAllTemplateFormAssignmentAction,
...
//   let interviewerBatches = useSelector(
//     (state) => state.template_interviewerDetails.batchesOfInterviewer
//   );
    ...
    getInterviewerList("get-interviewer-list").then((result) => {
      ...
      if (interviewers.length > 0) {
        ...
        if (singleInterviewerDetails.length === 0) {
          ...
          dispatch(
            saveAllTemplateFormAssignmentAction(
              initialValues.templateAssignmentFormsList
            )
          );
        //   dispatch(saveAllBatchesOfInterviewerAction(initialValues.batches));
        }
      }
    });
      ...
      <div className="formComponent  flex flex-direction-column align-items-center justify-content-center">
        ...
        <div className="interviewerDetailParentContainer flex flex-direction-column align-items-center">
          <div className="details">
            <label>Name:</label>
            <p
              className="single_interviewer_details detail_heading"
          ...
          <div className="details">
            ...
            <p
              className="single_interviewer_details sub_detail_heading"
              title="Email ID"
            >
          ...
          <div
            className="interviewer_templates_container flex flex-direction-column"
            title="Template Assignment Set"
          >
            <div className="mxy-2">Template Details</div>
            {initialValues.templateAssignmentFormsList
              .map((templateSet, tIndex) => {
                return (
                  <div className="templateSetCard border-white" key={tIndex}>
                    <div className="templateCardHeading" title="Template Name">
                      <Link
                        to={`/mock/single/template/detail/${templateSet._id}`}
                      >
                    ...
                    <div className="flex flex-no-wrap justify-content-flex-end align-content-center">
                      <span
                        ...
                        onClick={() => removeAssignedTemplateFromList(tIndex)}
            ...
            <div className="count_details bg-info">
              Total No. of Templates:{" "}
              {initialValues.templateAssignmentFormsList.length}
            </div>
          </div>

          <div
            className="interviewerBatchContainer flex flex-direction-column align-content-center"
            title="Batch List"
          >
            <div className="mxy-2 iBParentContainer">Batch Details</div>
            {initialValues.batches.map((batch, bIndex) => {
              return (
                  <div
                    className="interviewerBatchCardHeading  primary-color"
                    title="Batch Name" key={bIndex}
                  >
                    {bIndex + 1} {batch.batch_name}
                  </div>
              );
            })}
            <div className="count_details bg-warning">Batch Count: {initialValues.batches.length}</div>
          </div>
          ...
          <div className="form-buttons">
            ...
            <button
              onClick={() => history.push("/mock/template/interviewer/list")}
            >
              <span className="backIcon">
                <i className="fa fa-hand-o-left" aria-hidden="true"></i>{" "}
              </span>{" "}
