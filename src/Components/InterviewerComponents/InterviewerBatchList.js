...
function InterviewerBatchList({ history, user }) {
  ...
  let [viewStudents, setViewStudents] = useState(false);
  let [batchIDStudents,setBatchIDStudents]=useState();
                ...
                return (
                  ...
                  >
                    ...
                    <div className="batchListIcons flex justify-content-space-around align-items-center">
                      ...
                      <span title="View Students" onClick={(batch) => {
                        setViewStudents(true)
                      setBatchIDStudents=(batch.batch_id)
                        }
                         }>
                      <i className="fa fa-users success-color" aria-hidden="true"></i>
                    </span>
                    {viewStudents ? < Interviewer_Batch_Students open={viewStudents} batch_id={batchIDStudents}>
                      <button className="btn btn-danger" onClick={() => setViewStudents(false)}>X</button>
                    </Interviewer_Batch_Students> : null}
                  </div>
                  </div>
            );
              })}
